 ...
 * -----------------------------------------------
 * 2013/02/08	     V1.0	  韦东山	      创建
 * 2017/09/23        V2.0     TECH-PRO		  修改，支持多种图片格式
 ***********************************************************************/
int GetPixelDatasForIcon(char *strFileName, PT_PixelDatas ptPixelDatas)
{
	...
	char *pcStrFileNameSuffix;		/* Add by lele. */

	pcStrFileNameSuffix = strchr(strFileName, '.');
	if(pcStrFileNameSuffix == NULL)
	{
		DBG_PRINTF("The file name is invalid!\n");
        return -1;
	}
	pcStrFileNameSuffix += 1;	/* 让它从后缀开始 */
	...
	if(strncmp(pcStrFileNameSuffix, "bmp", 3) == 0)
    	iError = Parser("bmp")->isSupport(&tFileMap);
	else if(strncmp(pcStrFileNameSuffix, "jpg", 3) == 0)
		iError = Parser("jpg")->isSupport(&tFileMap);
	else 
		iError = 0;
    if (iError == 0)
    {
	...
	if(strncmp(pcStrFileNameSuffix, "bmp", 3) == 0)
		...
		iError = Parser("jpg")->GetPixelDatas(&tFileMap, ptPixelDatas);
	else 
		iError = -1;
    if (iError)
    {
