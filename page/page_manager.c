...
/**********************************************************************
 * 函数名称： GeneratePageAndMidPath
 * 功能描述： 从图标文件中解析出图像数据并放在指定区域,从而生成页面数据
 * 输入参数： ptPageLayout - 内含多个图标的文件名和显示区域
 *            ptVideoMem   - 在这个VideoMem里构造页面数据
 *			  strPath	   - 添加一个中间的子目录，来区分不同图标的路径
 * 输出参数： 无
 * 返 回 值： 0      - 成功
 *            其他值 - 失败
 * 修改日期        版本号     修改人	      修改内容
 * -----------------------------------------------
 * 2013/02/08	     V1.0	  韦东山	      创建
 * 2017/10/10        V2.0     TECH-PRO		  修改
 ***********************************************************************/
int GeneratePageAndMidPath(PT_PageLayout ptPageLayout, PT_VideoMem ptVideoMem, char *strPath)
{
	T_PixelDatas tOriginIconPixelDatas;
	T_PixelDatas tIconPixelDatas;
	int iError;
	PT_Layout atLayout = ptPageLayout->atLayout;
	char strPathAndIconName[128];
	...
	if (ptVideoMem->ePicState != PS_GENERATED)
	{
		...
		{
			/* 清空这块内存 */
			memset(strPathAndIconName, 0, sizeof(strPathAndIconName)/sizeof(char));
			snprintf(strPathAndIconName, 128, "%s/%s", strPath, atLayout->strIconName);
			strPathAndIconName[127] = '\0';
			
            /* 取得图标文件的象素数据 */
			iError = GetPixelDatasForIcon(strPathAndIconName, &tOriginIconPixelDatas);
			if (iError)
			{
				DBG_PRINTF("GetPixelDatasForIcon %s error!\n", atLayout->strIconName);
                free(tIconPixelDatas.aucPixelDatas);
